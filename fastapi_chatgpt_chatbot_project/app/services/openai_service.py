...
NO_SAMPLING_PREFIXES = ["gpt-5", "o1", "o3"]
...
def is_reasoning_model(model: str) -> bool:
    # model이 NO_SAMPLING_PREFIXES 중 하나로 시작하면 True
    return any(model.startswith(prefix) for prefix in NO_SAMPLING_PREFIXES)


def build_request_params(model: str, messages: list, temperature: float, top_p: float, max_output_tokens: int) -> dict:
    params = {
        "model": model,
        "messages": messages,
