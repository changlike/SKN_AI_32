...
class Encoder(nn.Module):
    """입력 문장을 읽어서 문장 전체 의미를 은닉상태로 압축하는 인코더입니다."""
    ...
    def __init__(self, vocab_size, embed_size, hidden_size):
        ...
        # batch_first=True : 입력 텐서 형태를 (배치, 시간, 특성)으로 사용하겠다는 의미임
    ...
    def forward(self, source_idx):
        ...
        # 디코더는 주로 마지막 hidden을 사용하므로 hidden을 반환합니다.
        return hidden
# class Encoder ---------------------------------------------

class Decoder(nn.Module):
    """인코더 은닉 상태를 기반으로 번역 문장을 한글자씩 생성하는 디코더입니다."""
    def __init__(self, vocab_size, embed_size, hidden_size):
        # 반드시 첫줄에 부모 생성자 호출함
        super().__init__()
        ...
        self.gru = nn.GRU(embed_size, hidden_size, batch_first=True)
        # GRU 출력 벡터를 전체 문자 사전 크기의 점수(logis)로 변환하는 선형 계층
        self.fc = nn.Linear(hidden_size, vocab_size)

    def forward(self, decoder_input_idx, hidden):
        # 디코더 입력 글자 인덱스를 임베딩 벡터로 변환함
        embedded = self.embedding(decoder_input_idx)
        # 인코더에서 전달받은 hidden을 초기 상태로 사용해서 출력 시퀀스를 계산함
        outputs, hidden = self.gru(embedded, hidden)
        # 각 시점의 GRU 출력 벡터를 문자열 정수(logits)로 변환함
        logits = self.fc(outputs)
        # 학습에는 logits이 필요하고, 다음 단계 추론에는 hidden이 필요함
...
class Seq2SeqTranslator(nn.Module):
    ...
    def forward(self, source_idx, decoder_input_idx):
        ...
        # 디코더가 정답 문장의 이전 글자들을 입력받아 다음 글자 점수를 예측함
