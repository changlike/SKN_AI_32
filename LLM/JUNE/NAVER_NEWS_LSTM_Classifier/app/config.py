...
class Config:
    ...
    max_vocab: int = 8000          # 토큰화에 사용할 최대 단어 수이다.
    max_len: int = 30              # 모든 기사 문장의 길이를 동일하게 맞추기 위한 최대 토큰 길이이다.
    embed_dim: int = 128            # 각 단어 정수를 몇 차원의 임베딩 벡터로 바꿀지 정한다.
    hidden_dim: int = 128           # LSTM 내부 은닉 상태의 차원 수이다.
    batch_size: int = 16            # 한 번의 학습 단계에서 모델에 넣을 샘플 개수이다.
    epochs: int = 20                # 전체 학습 데이터를 몇 번 반복해서 학습할지 정한다.
