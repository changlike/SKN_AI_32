# -*- coding: utf-8 -*-
"""임원용 매출 보고서: 집계·서술·저장 파이프라인입니다.

이전 강의(제24강)의 sales_report.py를 이 FastAPI 프로젝트 구조로 이식한 버전입니다.
변경점은 아래 3곳뿐이고, 집계·프롬프트·저장 로직은 원본과 동일합니다.
  1) common / app_context / message_utils → app.core.common / app.core.settings / app.services.llm_response
  2) get_provider() 전역 상태 제거 → provider를 모든 함수 인자로 직접 전달
     (FastAPI는 요청마다 동시에 처리되므로 전역 변수 공유는 요청 간 값이 섞일 위험이 있습니다)
  3) ROOT/DATA → PROJECT_ROOT/DATA_DIR (settings.py에 이미 정의된 경로 재사용)
"""

# datetime은 보고서 저장 헤더에 생성일을 기록합니다.
import datetime
# Path는 저장 경로를 다룹니다.
...
# pandas는 월간 매출 CSV를 DataFrame으로 읽고 집계합니다.
import pandas as pd

# 공용 LLM 생성 함수를 가져옵니다 (원본의 common.get_chat과 동일 함수입니다).
...
# 기본 분석 파일을 data 폴더의 monthly_sales.csv로 지정합니다.
DEFAULT_SALES_PATH = DATA_DIR / "monthly_sales.csv"
...
# write_report 함수는 provider를 인자로 직접 받습니다 (원본의 get_provider() 전역 조회 제거).
...
def write_report_with_fallback(facts: dict, provider: Literal["openai", "gemini"]) -> str:
    ...
    try:
        ...
        logger.exception("임원용 매출 보고서 LLM 생성 실패: provider=%s", provider)
...
def save_report(facts: dict, body: str, filename: Optional[str] = None) -> Path:
    ...
    # 저장 완료를 구조적 로그로 남깁니다 (원본은 콘솔 print였던 부분).
    logger.info("임원용 매출 보고서 저장 완료: path=%s", report_path)
    # 호출부가 저장 결과를 응답하거나 후속 처리할 수 있도록 경로를 반환합니다.
    return report_path


# generate_monthly_report 함수는 provider를 받아 집계→서술→저장을 한 번에 실행합니다.
def generate_monthly_report(month: Optional[str], provider: Literal["openai", "gemini"]) -> dict:
    ...
    # 완성된 보고서를 저장합니다.
    report_path = save_report(facts, body)
