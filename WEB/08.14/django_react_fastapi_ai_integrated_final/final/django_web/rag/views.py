"""Django 화면과 FastAPI AI 파이프라인 사이를 연결하는 Class Based View입니다."""
...
# 클래스 기반 View에서 로그인 사용자에게만 AI 기능을 허용하기 위한 Mixin을 가져옵니다.
from django.contrib.auth.mixins import LoginRequiredMixin
# JSON API 응답과 파일 응답을 만들기 위한 클래스를 가져옵니다.
...
class RagSearchView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # GET 요청에서는 빈 RAG 질문 폼을 생성합니다.
        form = RagQueryForm()
        # 기존 템플릿 컨텍스트 키를 그대로 유지합니다.
        return render(request, "rag/search.html", {"form": form, "answer": None, "results": [], "sync_result": None})

    def post(self, request):
        # POST 요청의 입력값을 폼에 바인딩합니다.
...
class RagDocumentView(LoginRequiredMixin, View):
...
class MultimodalPageView(LoginRequiredMixin, View):
...
class AIHealthView(LoginRequiredMixin, View):
...
class AICaptionView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # 기존 함수형 View와 동일하게 POST 외 요청에 405 JSON 응답을 반환합니다.
        return JsonResponse({"detail": "POST 요청만 허용됩니다."}, status=405)


class AIGenerateView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        try:
            ...
            # 빈 seed 등 빈 값은 FastAPI에서 기본값을 쓰도록 전송 항목에서 제거합니다.
...
class AISTTView(LoginRequiredMixin, View):
