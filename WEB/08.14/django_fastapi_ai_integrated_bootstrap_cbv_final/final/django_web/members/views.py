"""회원가입, 로그인 세션, 회원정보, 관리자 회원관리 Class Based View를 정의합니다."""
# 사용자 인증/로그인/로그아웃 처리를 위한 함수를 가져옵니다.
from django.contrib.auth import login, logout
# 기존 관리자 권한 데코레이터의 리다이렉트 동작을 클래스 dispatch에 그대로 적용합니다.
from django.contrib.auth.decorators import user_passes_test
# 클래스 기반 View에서 로그인 사용자만 접근하도록 제한하는 Mixin을 가져옵니다.
...
# 함수형 데코레이터를 클래스 기반 View의 dispatch 메서드에 적용하기 위해 가져옵니다.
...
class SignUpView(View):
    ...
    def post(self, request):
        ...
        if request.user.is_authenticated:
            return redirect("home")
        # POST 요청의 일반 값과 업로드 파일을 폼에 바인딩합니다.
        ...
        # 검증 오류가 있으면 오류 정보가 포함된 동일 폼을 다시 출력합니다.
        return render(request, "members/signup.html", {"form": form})
...
class LoginView(View):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            return redirect("boards:list")
        # GET 요청에서는 비어 있는 로그인 폼을 생성합니다.
        form = LoginForm(request=request, data=None)
        # 로그인 화면을 출력합니다.
    ...
    def post(self, request):
        ...
        # 로그인 실패 시 오류 정보가 포함된 동일 폼을 다시 출력합니다.
        return render(request, "members/login.html", {"form": form})
...
class LogoutView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # 기존 함수형 View와 동일하게 GET 요청에서는 세션을 삭제하지 않고 메인으로 이동합니다.
        return redirect("home")
...
class ProfileUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # 기존 회원 객체를 instance로 전달하여 현재 값이 채워진 수정 폼을 생성합니다.
        form = ProfileUpdateForm(instance=request.user)
        # 수정 폼을 출력합니다.
        return render(request, "members/profile_form.html", {"form": form})

    def post(self, request):
        # POST 입력값과 업로드 파일을 현재 로그인 회원 객체에 바인딩합니다.
        form = ProfileUpdateForm(request.POST, request.FILES, instance=request.user)
        # 유효성 검사를 통과하면 DB에 UPDATE SQL에 해당하는 ORM 작업을 수행합니다.
        if form.is_valid():
            form.save()
            messages.success(request, "회원 정보가 수정되었습니다.")
            return redirect("members:profile")
        # 검증 오류가 있으면 오류 정보가 포함된 폼을 다시 출력합니다.
        return render(request, "members/profile_form.html", {"form": form})
...
class WithdrawView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # GET 요청에서는 실제 탈퇴를 수행하지 않고 확인 화면만 출력합니다.
...
def _is_admin(user):
    ...
    # 기존 함수형 View에서 사용하던 조건식을 그대로 유지합니다.
    return user.is_authenticated and (user.is_staff or user.is_superuser)


@method_decorator(user_passes_test(_is_admin), name="dispatch")
class MemberListView(View):
    ...
    # 기존 함수형 View는 HTTP 메서드 종류와 관계없이 같은 목록 화면을 반환했으므로 동일하게 처리합니다.
    post = get
    put = get
    patch = get
    delete = get
...
class ToggleActiveView(View):
    ...
    def get(self, request, pk):
        # 기존 함수형 View와 동일하게 GET에서는 상태를 변경하지 않고 회원 목록으로 이동합니다.
    ...
    # 기존 함수형 View에서 POST가 아닌 요청은 상태 변경 없이 목록으로 이동했습니다.
    put = get
    patch = get
    delete = get
