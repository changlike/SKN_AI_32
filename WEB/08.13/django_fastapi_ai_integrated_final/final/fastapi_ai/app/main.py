...
# Django 프론트엔드의 직접 API 호출도 허용할 수 있도록 CORS 미들웨어를 가져옵니다.
...
# Django와 공유하는 MySQL 연결 상태를 점검하는 함수를 가져옵니다.
from app.database import check_database

# 문서/게시글 RAG REST API 라우터를 가져옵니다.
...
# Django 개발 서버 주소를 허용하여 브라우저가 FastAPI를 직접 호출하는 경우에도 CORS 오류가 발생하지 않게 합니다.
app.add_middleware(
    CORSMiddleware,
...
# /api/rag 이하의 RAG 엔드포인트를 메인 FastAPI 애플리케이션에 연결합니다.
...
async def health() -> dict[str, str]:
    ...
    # AI 서버와 Django가 사용하는 MySQL 연결 상태를 함께 반환합니다.
    return {"status": "ok", "message": "FastAPI AI server is running", "database": "ok" if database_ok else "error"}
